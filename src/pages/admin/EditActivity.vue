  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        ...
        <p class="text-muted-foreground mt-1">Update activity details</p>
      </div>
    </div>

    <!-- Loading State -->
    <div v-if="fetching" class="card flex items-center justify-center py-12">
...
async function fetchActivity() {
  fetching.value = true
  try {
    const response = await activitiesAPI.getById(route.params.id)
    const activity = response.data
    
    form.title = activity.title || activity.nom || ''
    form.description = activity.description || ''
    form.date = activity.date ? activity.date.split('T')[0] : ''
    form.location = activity.location || activity.lieu || ''
    form.guestAccessible = activity.accessibleInvite || activity.guestAccessible || false
  } catch (err) {
...
async function handleSubmit() {
  ...
  try {
    await activitiesAPI.update(route.params.id, {
      nom: form.title,
      title: form.title,
